// src/app/api/ppid/track-keberatan/route.ts
import { NextRequest, NextResponse } from "next/server";
import { getKeberatanByNomor } from "@/lib/queries/ppid";
...
export async function GET(req: NextRequest) {
  ...
  const data = await getKeberatanByNomor(nomor);
  if (!data) {
  ...
  return NextResponse.json({
    nomorKeberatan: data.nomorKeberatan,
    namaPemohon: data.namaPemohon,
    alasanKeberatan: data.alasanKeberatan,
    status: data.status,
